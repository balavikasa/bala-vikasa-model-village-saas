...
def page():
    return render_template("admin_transfer.html", resources=resource_catalog())
...
def export_master():
    ...
    try:
        workbook = build_export_workbook(resource, current_user._get_current_object())
        buffer = BytesIO()
...
def preview():
    ...
    try:
        ...
        try:
            token, result = stage_import(temp, resource, current_user._get_current_object())
        finally:
            temp.unlink(missing_ok=True)
        return jsonify(token=token, preview=result)
    except MasterTransferError as exc:
        return jsonify(error=str(exc)), 422
