...
def update_monthly_plan(plan_id: int):
    try:
        ...
        if not can_manage_action_plan(
            ...
            raise ScopeError(
                "You cannot change this monthly plan."
            )

        if plan_locked(plan):
            raise PlanningError(
                "This monthly plan is locked by immutable history."
            )
        ...
        if assigned_raw and not plan_type_raw:
            raise PlanningError(
                "Choose Attendance or Specials before assigning a date."
            )

        plan_type = (
            ActionPlanType(plan_type_raw)
            if plan_type_raw
            else None
        )
        ...
        if assigned_raw:
            ...
            assigned_date = date.fromisoformat(
                assigned_raw
            )

            if not plan.plan_month or (
                assigned_date.year,
                assigned_date.month,
            ) != (
                ...
                raise PlanningError(
                    "Assigned Date must be inside "
                    "the selected plan month."
                )
...
def create_monthly_plan():
    try:
        ...
        if not can_manage_action_plan(
            ...
            raise ScopeError(
                "You cannot assign a plan for that committee."
            )
...
def export_month():
    ...
    try:
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=(
                f"Action_Plans_{month_key(selected)}.xlsx"
            ),
        )
...
def import_preview():
    ...
    if not upload or not upload.filename:
        return jsonify(
            error="Choose an .xlsx action-plan workbook."
        ), 422

    if not upload.filename.lower().endswith(".xlsx"):
        return jsonify(
            error=(
                "Only .xlsx action-plan "
    ...
    try:
        ...
        # Keep uploaded workbook outside executable/static paths.
        ...
        current_app.logger.info(
            "Action-plan import preview created "
            "for user_id=%s month=%s token_prefix=%s",
            current_user.id,
            month_key(selected),
            token[:8],
        )
        ...
        current_app.logger.warning(
            "Action-plan import preview rejected "
            "for user_id=%s: %s",
            current_user.id,
            exc,
        )
        ...
        current_app.logger.exception(
            "Unexpected action-plan import "
            "preview failure for user_id=%s.",
            current_user.id,
        )

        return jsonify(
            error=(
                "The workbook could not be validated because "
...
def import_confirm():
    ...
    try:
        ...
        current_app.logger.info(
            "Action-plan import confirmation started "
            "for user_id=%s month=%s token_prefix=%s",
            current_user.id,
            month_key(selected),
            token[:8],
        ...
        current_app.logger.info(
            "Action-plan import confirmation completed "
            "for user_id=%s month=%s token_prefix=%s",
            current_user.id,
            month_key(selected),
            token[:8],
        ...
        current_app.logger.warning(
            "Action-plan import confirmation rejected "
            "for user_id=%s token_prefix=%s: %s",
            current_user.id,
        ...
        current_app.logger.exception(
            "Unexpected action-plan import confirmation "
            "failure for user_id=%s token_prefix=%s.",
            current_user.id,
            token[:8] if token else "missing",
        )
        ...
        return jsonify(
            error=(
                "The import could not be confirmed because "
                "the server encountered an unexpected error. "
                "No rows were saved."
            )
        ), 500